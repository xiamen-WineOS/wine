<template>
  <el-dialog top="20px" :visible.sync="dialogVisible">
    <article class="article-content">
      <header>
        <div class="h-top">
          <span class="bookmark">{{typeStr}}</span>
        </div>
        <!--<h2 v-text="ar.title">说好一起读书何不如期赴约</h2>-->
        <div class="h-bottom">
          <!--<a>{{user.userName}}</a>
          <span>{{post.createDate | dateFormat}}</span>-->
        </div>
      </header>
      ...
      <!--<div v-if="ar.tags && ar.tags.length > 0" class="ar-tags">
        <div class="color-secondary font-s12 margin-b">相关标签</div>
        <el-tag v-for="tag in ar.tags" :key="tag" type="gray">{{tag}}</el-tag>
      </div>-->
